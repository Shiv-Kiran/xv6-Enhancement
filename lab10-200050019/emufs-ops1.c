...
struct file_t
{
    ...
    int offset;       // offset of the file
                      // offset is for lseek to emufs_seek and other api.
...
{
    /*
    Creating new file system at mount_point
        * Read the superblock.
...
{
    // This create root for new mount.
    /*
        ...
        * Return value: -1,		            error (no free handles)
        ...
        Need to parse through dir and return the value i for which
        dir[i].mount_point = mount_point.
        If not there create.
    */
...
{
    /*
    // Just need to change dir[dir_handle].inode_number from return value.
...
{
    /*
    just point file[file_handle].mount_point, inode_um to return value.
    by alloc_file_handle;
...
{
    /*
    // Like file or directory create.
    Need to call read_inode, write_inode.
    ...
    */
    struct inode_t inode;
    read_inode(dir[dir_handle].mount_point, dir[dir_handle].inode_number, &inode); // reading current directory inode
    if (inode.size == MAX_FILE_SIZE)                                               // if all inodes are occupied.
        return -1;
    for (int i = 0; i < inode.size; i++)
    { // checking for name and type
        struct inode_t entry;
        read_inode(dir[dir_handle].mount_point, inode.mappings[i], &entry);
        if (memcmp(name, entry.name, MAX_ENTITY_NAME) == 0 && entry.type == type)
            return -1;
    }
    int inodenum = alloc_inode(dir[dir_handle].mount_point); // creating new inode
    if (inodenum == -1)
        return -1;
...
{
    /*
    File reading using read_datablock
    ...
    */
    ...
    int ptr = offset % BLOCKSIZE;
    int ptr2 = 0;
    char buffer[BLOCKSIZE];
    while (size > 0)
    {
        read_datablock(files[file_handle].mount_point, inode.mappings[block], buffer);
        while (ptr < BLOCKSIZE && size > 0)
        {
            buf[ptr2] = buffer[ptr];
            ptr++;
            ptr2++;
            size--;
            files[file_handle].offset += 1;
        }
        block++;
        ptr = 0;
    }
...
{
    /*
    Just moving offset of files[file_handle].offset
