...
void signal_sem(void *chan)
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    ...
    {
      p->state = RUNNABLE;
      // cprintf("Changed proc state \n");
    }
  // cprintf("Checking for lock %d \n", ((struct lock_t *)chan)->locked);

  release(&ptable.lock);
  // cprintf("Released ptable \n");
  // acquire_lock((struct lock_t *)chan);
}